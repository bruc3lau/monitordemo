...
- **中文：** 部署在目标节点上用于采集服务器指标的代理（收集 CPU、内存、网络IO 和英伟达显卡使用率），并通过定时 POST 请求发送到后端。
...
- **中文：** 管理端后端服务，负责使用 `chi` 构建轻量级 REST API，接收各个代理节点发送的资源数据并在内存中缓存历史状态记录，为前端提供接口调用。
...
- **中文：** 炫酷的暗黑风格实时监控大屏前端，基于 Vite + React 搭建，采用 Tailwind CSS v4 样式和 Recharts 进行数据可视化展示。
